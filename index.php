	...
	if(password_verify($password, $victory)){
		$_SESSION["email"] = $email;
			...
			$sql2 = "SELECT status FROM users WHERE email = '$email'";
			$fetch2 = mysql_query($sql2);
			$res = mysql_fetch_object($fetch2);
			$status = $res->status;
		
		$_SESSION["status"] = $status; 
		
			$sql3 = "SELECT username FROM users WHERE email = '$email'";
			$fetch3 = mysql_query($sql3);
			$res = mysql_fetch_object($fetch3);
